...
$('document').ready(function() {
  ...
  $('svg .feature').on('click touchend', function(evt) {
    ...
    var left_offset = last_coord.x - 10
    if(left_offset + $("#popup").outerWidth() > window.innerWidth)
      left_offset = window.innerWidth - $("#popup").outerWidth() - 10
    if(left_offset < 0) left_offset=0

    $('#popup')
      .css({left: left_offset, top: last_coord.y - 180})
      .show();
...
function doDrag(evt) {
  ...
  $('#popup').css({left: poppos.left + offset.x, top: poppos.top + offset.y});
...
function renderNotes() {
  $('#released').append($("<div class='control close'></div><h1>Release Notes</h1>"));

  for (var i = 0; i < releases.length; i++) {
    var note = releases[i];
    var nhtml = $('<div class=release></div>');
    nhtml.append($('<div class=version>' + (note.version ? 'v' + note.version + ' | ' : '') + note.type + '</div>'));
    nhtml.append($('<h2>' + note.date + '</h2>'));
    //nhtml.append($("<div class=type>"+note.type+"</div>"))
    nhtml.append($('<div class=desc>' + note.notes + '</div>'));

    $('#released').append(nhtml);
  }
}

function renderCalendar() {
  $('#calendar').append($("<div class='control close'></div><h1>Delivery Plan</h1>"));
  ...
  if (developing.length > 0) {
    ...
    for (var i = 0; i < developing.length; i++) {
      var card = developing[i];
      var nhtml = $('<div class=feature></div>');
      nhtml.append('<h3>' + card.title + '</h3>');
      nhtml.append('<div class=estimate>' + card.desc + '</div>');
      $('#calendar').append(nhtml);
    }
  }

  if (planned.length > 0) {
    $('#calendar').append('<h2>Starting Soon</h2>');
...
function quarterly(input) {
  var th = ['', '1st', '2nd', '3rd', '4th'];
  var segs = input.toLowerCase().split('q');
  return th[segs[1]] + ' qtr ' + segs[0];
}
...
function renderCard(card) {
  ...
  popup.append($('<h3>' + card.title + '</h3>'));
  popup.append($('<div class=status>' + card.status + (card.estimate ? ' - ' + card.estimate : '') + '</div>'));
  popup.append($('<div class=desc>' + card.desc + '</div>'));

  return;
}

function getCard(id) {
  for (var i = 0; i < cards.length; i++) if (id.toLowerCase() == cards[i].id.toLowerCase()) return cards[i];
}
